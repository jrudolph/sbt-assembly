...
object IO2 {
  ...
  /** File scheme name */
  private[sbtassembly] val FileScheme = "file"
  ...
  private[sbtassembly] def jars(dir: File): Iterable[File] = listFiles(dir, GlobFilter("*.jar"))
  ...
  private[sbtassembly] def wrapNull(a: Array[File]) =
    if (a == null)
      new Array[File](0)
    else
      a
   ...
   */
  def jar(sources: Traversable[FileEntry], outputJar: File, manifest: Manifest): Unit =
   ...
   */
  def zip(sources: Traversable[FileEntry], outputZip: File): Unit =
    archive(sources.toSeq, outputZip, None)

  private def archive(sources: Seq[FileEntry], outputFile: File, manifest: Option[Manifest]): Unit = {
  ...
  private def writeZip(sources: Seq[FileEntry], output: ZipOutputStream)(createEntry: String => ZipEntry): Unit = {
    val files = sources.map(f => f.copy(path = normalizeName(f.path)))
    ...
    def makeFileEntry(lastModified: Long, name: String) =
    {
    ...
    allDirectoryPaths(files.map(_.path)) foreach addDirectoryEntry
  ...
  private def allDirectoryPaths(files: Iterable[String]) =
